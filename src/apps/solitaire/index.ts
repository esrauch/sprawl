...
type Zone = 'waste' | 'foundation' | 'tableau';
...
function canMove(card: Card, targetTop: Card | null, zone: 'foundation' | 'tableau'): boolean {
    if (zone === 'foundation') {
        if (!targetTop) return card.rank === 1;
        return card.suit === targetTop.suit && card.rank === targetTop.rank + 1;
    } else {
        if (!targetTop) return card.rank === 13; // King
...
function removeSelectedCards() {
    if (!selection) return;
    if (selection.zone === 'waste') state.waste.pop();
    if (selection.zone === 'foundation') state.foundations[selection.index].pop();
    if (selection.zone === 'tableau') {
        const pile = state.tableaus[selection.index];
...
function tryAutoMoveToFoundation(card: Card): boolean {
    for (let i = 0; i < 4; i++) {
...
function handleTap(zone: Zone, pileIndex: number, cardIndex: number) {
    if (selection) {
...
function isSelected(zone: Zone, index: number, cardIndex?: number): boolean {
    if (!selection) return false;
    if (selection.zone !== zone || selection.index !== index) return false;
    if (zone === 'tableau') return cardIndex! >= selection.cardIndex!;
    return true;
}
...
function createCardElement(card: Card, selected: boolean) {
    const el = document.createElement("div");
