...
class SolitaireApp implements AppInstance {
    ...
    private canMove(card: Card, targetTop: Card | null, zone: 'foundation' | 'tableau'): boolean {
        if (zone === 'foundation') {
    ...
    private tryAutoMoveToFoundation(card: Card): boolean {
        for (let i = 0; i < 4; i++) {
    ...
    private handleTap(zone: Zone, pileIndex: number, cardIndex: number): void {
        if (this.selection) {
    ...
    private isSelected(zone: Zone, index: number, cardIndex?: number): boolean {
        if (!this.selection) return false;
        if (this.selection.zone !== zone || this.selection.index !== index) return false;
        if (zone === 'tableau') return cardIndex! >= this.selection.cardIndex!;
        return true;
    }
    ...
    private createCardElement(card: Card, selected: boolean) {
        const el = document.createElement("div");
