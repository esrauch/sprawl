...
// ── Main app ─────────────────────────────────────────────────
class GolfApp implements AppInstance {
    ...
    // Game state
    private ball: Ball = { x: 0, y: 0, vx: 0, vy: 0, resting: true, restTimer: 0 };
    private terrain: Point[] = [];
    private hole: Hole = { x: 0, y: 0, w: 0, h: 0 };
    
    private currentHole = 1;
    private strokes = 0;
    private state: "PLAYING" | "HOLED" | "TRANSITION" = "PLAYING";
    private transitionTimer = 0;
    private generatedNext = false;
    ...
    // ── Input ────────────────────────────────────────────────
    private getLogicalPos(e: PointerEvent): {x: number, y: number} {
    ...
    private onPointerDown(e: PointerEvent) {
        if (this.state !== "PLAYING" || !this.ball.resting) return;
    ...
    private updatePhysics(dt: number) {
        ...
        for (let i = 0; i < SUBSTEPS; i++) {
            ...
            // Wall bounces (sides)
            if (b.x < BALL_RADIUS) { b.x = BALL_RADIUS; b.vx *= -BOUNCE; }
            if (b.x > LOGICAL_WIDTH - BALL_RADIUS) { b.x = LOGICAL_WIDTH - BALL_RADIUS; b.vx *= -BOUNCE; }
