...
interface Train {
    lineIdx: number;
    // Train ping-pongs through station list using a "stop index"
    // and direction. segIdx is the index of the station the train
    // is currently travelling FROM; it travels toward segIdx+1 (forward)
    // or segIdx (backward, from segIdx+1).
    segIdx: number;
...
// ── Main app ─────────────────────────────────────────────────
class BaseApp implements AppInstance {
    ...
    private ghostTrains: GhostTrain[] = [];
    ...
    private activeLineIdx: number = 0;
    ...
    // Bound handlers
    private handlePointer = (e: PointerEvent) => this.onTap(e);
    ...
    // ── Station management ───────────────────────────────────
    private addStation(nx: number, ny: number, shape: Shape) {
        this.stations.push({
    ...
    // ── Line management ──────────────────────────────────────
    private connectStations(aId: number, bId: number) {
        // Prevent duplicate direct connections
        for (const line of this.lines) {
            for (let i = 0; i < line.stationIds.length - 1; i++) {
                if (
                    (line.stationIds[i] === aId && line.stationIds[i + 1] === bId) ||
    ...
    private tryExtendLine(li: number, line: Line, aId: number, bId: number): boolean {
    ...
    private shiftTrainIndices(lineIdx: number, delta: number) {
        for (const tr of this.trains) {
    ...
    private updateSpawns(dt: number) {
        ...
        if (this.stationTimer >= stationInterval && this.stations.length < 16) {
            ...
            for (let attempt = 0; attempt < 20; attempt++) {
                ...
                let tooClose = false;
                for (const s of this.stations) {
                    if (dist(nx, ny, s.x, s.y) < 0.12) { tooClose = true; break; }
                }
                if (!tooClose) {
    ...
    private updateTrains(dt: number) {
        for (const tr of this.trains) {
            ...
            // Bounds safety
            if (fromIdx < 0 || fromIdx >= line.stationIds.length) continue;
            if (toIdx < 0 || toIdx >= line.stationIds.length) continue;
    ...
    private trainAtStation(tr: Train, stationId: number) {
        ...
        const upcomingShapes = new Set<Shape>();

        if (isCircular) {
            for (const id of line.stationIds) {
                const s = this.stationById(id);
                if (s) upcomingShapes.add(s.shape);
            }
        } else {
            if (tr.forward) {
    ...
    // ── Overflow check ───────────────────────────────────────
    private updateGhostTrains(dt: number) {
            ...
            if (gt.t >= 1) {
                ...
                if (st) {
                    for (const p of gt.passengers) {
                        if (p === st.shape) this.score++;
                        else st.passengers.push(p);
                    }
                }
    ...
    private drawShape(ctx: CanvasRenderingContext2D, x: number, y: number, shape: Shape, r: number, colour: string, filled: boolean) {
        ctx.strokeStyle = colour;
        ctx.fillStyle = colour;
        ctx.lineWidth = 2;
    ...
    private getTrainPos(tr: Train): { x: number; y: number; angle: number } | null {
        const line = this.lines[tr.lineIdx];
        if (!line || line.stationIds.length < 2) return null;

        const fromIdx = tr.forward ? tr.segIdx : tr.segIdx + 1;
        const toIdx = tr.forward ? tr.segIdx + 1 : tr.segIdx;
        if (fromIdx < 0 || fromIdx >= line.stationIds.length) return null;
        if (toIdx < 0 || toIdx >= line.stationIds.length) return null;
