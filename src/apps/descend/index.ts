...
// ── Main app ─────────────────────────────────────────────────
class DescendApp implements AppInstance {
    ...
    // Game state
    private player: Player = { x: 5, y: 3, vx: 0, vy: 0, r: 0.4, ammo: MAX_AMMO, grounded: false };
    private projectiles: Projectile[] = [];
    private enemyProjectiles: EnemyProjectile[] = [];
    private enemies: Enemy[] = [];
    private nextEnemyId = 0;
    private grid: Record<number, number[]> = {};
    private maxYGenerated = -1;
    private cameraY = 0;
    
    private gameOver = false;
    private score = 0; // max depth
    private points = 0;
    private combo = 0;

    // Input state
    private keys = { left: false, right: false, jump: false, jumpPressed: false };
    private touches = { left: false, right: false, jump: false };
    
    // UI Refs
    private btnJumpFire: HTMLButtonElement | null = null;
    ...
    private buildUI() {
        ...
        btnJump.addEventListener("pointerdown", () => {
            if (!this.touches.jump) this.keys.jumpPressed = true;
    ...
    private generateRow(y: number, safe = false) {
        ...
        if (!safe) {
            ...
            for (let x = 1; x < COLS - 1; x++) {
                const r = Math.random();
                if (r < 0.01) row[x] = BLOCK_TYPES.SOLID;
                else if (r < 0.03) row[x] = BLOCK_TYPES.BREAKABLE;
                else if (r < 0.04) this.enemies.push({ id: this.nextEnemyId++, type: "HAZARD", x: x + 0.5, y: y + 0.5, vx: 0, r: 0.45, shootTimer: Math.random() * 2, dead: false });
                else if (r < 0.06) this.enemies.push({ id: this.nextEnemyId++, type: "STOMPABLE", x: x + 0.5, y: y + 0.5, vx: Math.random() > 0.5 ? 2 : -2, r: 0.45, shootTimer: 0, dead: false });
            }
        }
    ...
    private getBlock(x: number, y: number): number {
        const ix = Math.floor(x);
        const iy = Math.floor(y);
        if (ix < 0 || ix >= COLS) return BLOCK_TYPES.SOLID;
        if (this.grid[iy]) return this.grid[iy][ix];
        return BLOCK_TYPES.EMPTY;
    }

    private setBlock(x: number, y: number, type: number) {
        const ix = Math.floor(x);
        const iy = Math.floor(y);
    ...
    private tick(now: number) {
        ...
        if (!this.gameOver) {
            ...
            for (const y of Object.keys(this.grid)) {
                if (parseInt(y) < topVisibleRow) delete this.grid[parseInt(y)];
            }
        }
    ...
    private updatePlayer(dt: number) {
        ...
        if (moveLeft && !moveRight) p.vx = -MOVE_SPEED;
        else if (moveRight && !moveLeft) p.vx = MOVE_SPEED;
        else p.vx = 0;
    ...
    private updateEnemyProjectiles(dt: number) {
        for (let i = this.enemyProjectiles.length - 1; i >= 0; i--) {
            ...
            if (this.getBlock(pr.x, pr.y) !== BLOCK_TYPES.EMPTY) pr.dead = true;
    ...
    private draw() {
        ...
        for (const [yStr, row] of Object.entries(this.grid)) {
            ...
            for (let x = 0; x < COLS; x++) {
                const b = row[x];
                if (b === BLOCK_TYPES.EMPTY) continue;
