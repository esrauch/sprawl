...
class AresApp {
    constructor() {
        ...
        this.camX = WORLD_W / 2;
        this.camY = WORLD_H * 0.65;
        this.camSpan = WORLD_H; // vertical extent visible
    ...
    tick(now) {
        ...
        if (!this.state.landed && !this.state.crashed) {
            // Rotation
            if (this.rotLeft)
                this.state.angle -= ROTATE_SPEED * dt;
            if (this.rotRight)
                this.state.angle += ROTATE_SPEED * dt;
            // Thrust
            ...
            if (this.state.x < 0) {
                this.state.x = 0;
                this.state.vx = Math.abs(this.state.vx) * 0.3;
            }
            if (this.state.x > WORLD_W) {
                this.state.x = WORLD_W;
                this.state.vx = -Math.abs(this.state.vx) * 0.3;
            }
            // Ceiling clamp
            if (this.state.y > WORLD_H) {
                this.state.y = WORLD_H;
                this.state.vy = 0;
            }
    ...
    draw() {
        ...
        const wx2sx = (wx) => ((wx - (this.camX - spanX / 2)) / spanX) * W;
        const wy2sy = (wy) => H - ((wy - (this.camY - spanY / 2)) / spanY) * H; // y-up → y-down
        // Clear
        ...
        if (this.state.landed || this.state.crashed) {
            ...
            ctx.fillText(this.state.landed ? "TOUCHDOWN CONFIRMED" : "MISSION FAILED", W / 2, H / 2 - 4);
